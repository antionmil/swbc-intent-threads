...
import { writeArtifact } from "@/lib/artifact";
...
const JOBS: Record<string, () => Promise<unknown>> = {
  ...
  refresh: async () => {
    // Replace per site. The shape is always: fetch -> compute -> writeArtifact.
    const data = { note: "replace me", at: new Date().toISOString() };
    return { artifact: await writeArtifact("main", data) };
  },
};
